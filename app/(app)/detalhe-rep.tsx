...
import { diaLegivel, diasNoMes, limitesDoMes, mesLegivel, somarMeses } from '@/lib/tempo';
...
/**
 * Desempenho de um rep no mês — metas, vendido e o detalhe por turno (com
 * vendido/meta/% por página nos turnos double). Conteúdo compartilhado por
 * `/admin/reps/[id]` e `/primaris/[id]`: mesmos dados, cada um com sua volta
 * e sua URL de navegação de mês — pra quem entra a partir de /primaris não
 * cair dentro do layout (e do menu) do admin.
 */
export async function DetalheRep({
  id,
  mes,
  basePath,
  voltarHref,
  ...
  id: string;
  mes: string;
  /** Prefixo da URL desta tela — usado nos links de navegação de mês. */
  basePath: string;
  voltarHref: string;
  voltarRotulo: string;
}) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href={voltarHref} className="text-sm text-texto-fraco hover:text-texto">
          ← {voltarRotulo}
        </Link>
      ...
      <div className="flex items-center gap-1 text-sm">
        <Link
          href={`${basePath}?mes=${somarMeses(mes, -1)}`}
